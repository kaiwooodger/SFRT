#!/usr/bin/env python3
"""Phase 36A: vessel falsification and surrogate sink comparison on the 10-case cohort."""
...
def build_true_vs_comparator_delta_rows(
    ...
    for plan_id in plan_ids:
        ...
        for comparator_id in PHASE36A_COMPARATOR_IDS:
...
def build_quick_assessment(
    ...
    key_comparators = {"mirror_lr_sink", "uniform_body_sink_mass_matched"}
    ...
    lines = [
        ...
        "- Interpretation: if the anatomical sink remains systematically different from mirrored or uniform surrogates, that supports an anatomy-specific rather than generic-washout explanation of the sink effect.",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    write_csv(out_root / "phase36a_endpoint_table.csv", endpoint_rows)
    write_csv(out_root / "phase36a_assay_table.csv", assay_rows)
    write_csv(out_root / "phase36a_model_metadata.csv", model_meta_rows)
    write_csv(out_root / "phase36a_true_vs_surrogate_endpoint_deltas.csv", endpoint_delta_rows)
    write_csv(out_root / "phase36a_true_vs_surrogate_assay_deltas.csv", assay_delta_rows)
    write_csv(out_root / "phase36a_falsification_endpoint_summary.csv", endpoint_summary_rows)
    write_csv(out_root / "phase36a_falsification_assay_summary.csv", assay_summary_rows)
    write_json(
        out_root / "phase36a_reproducibility_manifest.json",
        {
            "phase": "36A",
            "description": "Baseline vessel falsification and surrogate sink comparison on the Phase 33/34 cohort.",
    ...
    (out_root / "phase36a_quick_assessment.md").write_text(
        build_quick_assessment(endpoint_summary_rows, endpoint_rows),
        encoding="utf-8",
    )

    print("=== PHASE 36A VESSEL FALSIFICATION COHORT COMPLETE ===")
    print(f"Output root: {out_root}")
    print(f"Endpoint table: {out_root / 'phase36a_endpoint_table.csv'}")
    print(f"Endpoint summary: {out_root / 'phase36a_falsification_endpoint_summary.csv'}")
    print(f"Quick assessment: {out_root / 'phase36a_quick_assessment.md'}")
    return 0
