#!/usr/bin/env python3
"""Shared utilities for Phase 37 sink falsification analyses."""
...
MODEL_LABELS = {
    ...
    "ap_flip_sink": "Bystander, anterior-posterior flipped vessel sink",
    "si_flip_sink": "Bystander, superior-inferior flipped vessel sink",
    "local_dropout_sink_20mm": "Bystander, local vessel dropout within 20 mm of GTV",
    "randomized_displacement_sink": "Bystander, randomized anatomy-constrained vessel displacement",
...
PHASE37A_COMPARATOR_IDS: Tuple[str, ...] = (
    "no_sink",
    "ap_flip_sink",
    "si_flip_sink",
...
def _load_npz_with_retry(path: Path, *, attempts: int = 5, sleep_seconds: float = 0.5) -> np.lib.npyio.NpzFile:
    ...
    except Exception as exc:  # pragma: no cover - last-ditch fallback
        last_error = exc
    raise RuntimeError(f"Failed to load NPZ after retries: {path}") from last_error
...
def load_case_context(path: Path) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], Tuple[float, float, float], List[Tuple[float, float, float]]]:
    with _load_npz_with_retry(path) as data:
        axes_mm = {
...
def build_sink_uptake_tensor(
    ...
    *,
    ...
    dropout_radius_mm: float = 20.0,
    random_seed: int = 37,
    random_shift_min_mm: float = 6.0,
    random_shift_max_mm: float = 18.0,
    ...
    elif model_id == "ap_flip_sink":
        flipped = true_tensor[:, :, ::-1, :]
        uptake = _renormalize_species_mass(flipped, target_mass, support_mask=body)
    elif model_id == "si_flip_sink":
        flipped = true_tensor[:, :, :, ::-1]
        uptake = _renormalize_species_mass(flipped, target_mass, support_mask=body)
    elif model_id == "local_dropout_sink_20mm":
        distance = ndimage.distance_transform_edt(~gtv, sampling=spacing_mm)
        dropout_zone = body & (distance <= float(dropout_radius_mm))
        retained = true_tensor.copy()
        retained[:, dropout_zone] = 0.0
        if not np.any(retained > 0.0):
            support = body & (~dropout_zone)
            uptake = _uniform_mass_tensor(target_mass, support if np.any(support) else body)
        else:
            uptake = _renormalize_species_mass(retained, target_mass, support_mask=body)
    elif model_id == "randomized_displacement_sink":
        rng = np.random.default_rng(int(random_seed))
        choices_mm = np.linspace(float(random_shift_min_mm), float(random_shift_max_mm), num=7, dtype=np.float32)
        signs = np.asarray([-1.0, 1.0], dtype=np.float32)
        best_tensor = None
        best_support = -1
        best_shift = (0, 0, 0)
        original_support = int(np.count_nonzero(np.any(true_tensor > 0.0, axis=0)))
        for _ in range(32):
            shift_mm = []
            for axis_mm in spacing_mm:
                magnitude = float(rng.choice(choices_mm))
                signed_value = float(magnitude * float(rng.choice(signs)))
                shift_mm.append(signed_value)
            shift_vox = tuple(
                int(round(float(mm) / max(float(axis_mm), 1.0e-6)))
                for mm, axis_mm in zip(shift_mm, spacing_mm)
            )
            candidate = np.zeros_like(true_tensor, dtype=np.float32)
            for species_idx in range(true_tensor.shape[0]):
                candidate[species_idx] = _shift_3d_no_wrap(true_tensor[species_idx], shift_vox)
            candidate = _renormalize_species_mass(candidate, target_mass, support_mask=body)
            support_count = int(np.count_nonzero(np.any(candidate > 0.0, axis=0)))
            if support_count > best_support:
                best_tensor = candidate
                best_support = support_count
                best_shift = shift_vox
            if support_count >= max(1, int(0.65 * original_support)):
                break
        uptake = best_tensor if best_tensor is not None else true_tensor.copy()
    ...
    if model_id == "randomized_displacement_sink":
        meta["random_seed"] = int(random_seed)
        meta["shift_voxels_xyz"] = ",".join(str(int(value)) for value in best_shift)
    if model_id == "local_dropout_sink_20mm":
        meta["dropout_radius_mm"] = float(dropout_radius_mm)
    return uptake, meta
