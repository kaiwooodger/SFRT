...
from phase37_bio_model_params import (
    DEFAULT_BIO_MODEL_PARAMS,
    BioModelParams,
    bio_model_params_from_config,
)
...
def bio_args_from_config(config: Mapping[str, object], *, uptake_scale: float = 1.0) -> SimpleNamespace:
    params = bio_model_params_from_config(config, uptake_scale=float(uptake_scale))
    return bio_args_from_params(params)


def bio_args_from_params(params: BioModelParams) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        artery_ros_uptake=float(params.artery_ros_uptake),
        artery_cyto_uptake=float(params.artery_cyto_uptake),
        vein_ros_uptake=float(params.vein_ros_uptake),
        vein_cyto_uptake=float(params.vein_cyto_uptake),
    )
...
def build_emission_tensor(
    dose_grid: np.ndarray,
    *,
    params: BioModelParams,
    m_type: np.ndarray,
    m_oxygen: np.ndarray,
) -> np.ndarray:
    base_emission = (1.0 - np.exp(-float(params.gamma) * np.asarray(dose_grid, dtype=np.float32))).astype(np.float32)
    return (
        np.asarray([float(params.emax_ros), float(params.emax_cyto)], dtype=np.float32)[:, None, None, None]
        * base_emission[None, :, :, :]
...
def evaluate_bystander_mode(
    *,
    ...
    verbose_pde: bool,
    bio_params: BioModelParams | None = None,
) -> Dict[str, object]:
    params = bio_params or bio_model_params_from_config(config, uptake_scale=float(uptake_scale), include_immune=False)
    alpha = float(params.alpha)
    beta = float(params.beta)
    bio_args = bio_args_from_params(params)
    ...
    if verbose_pde:
        sink_state = "vascular_sink" if mode == "bystander_with_sink" else "no_vascular_sink"
        active_terms = ["ros", "cytokine", sink_state]
        if float(params.weight_immune) > 0.0:
            active_terms.append("immune")
        print(
            "Active model terms: "
            + ", ".join(active_terms)
            + f" | weight_immune={float(params.weight_immune):.1f}"
        )
    ...
    emission_tensor = build_emission_tensor(physical_dose, params=params, m_type=m_type, m_oxygen=m_oxygen)
    observables = solve_multispecies_pde_3d_with_hazard_observables(
        physical_dose,
        voxel_size_mm,
        diffusion_coeffs=(float(params.diffusion_ros), float(params.diffusion_cyto)),
        decay_coeffs=(float(params.decay_ros), float(params.decay_cyto)),
        emission_emax=(float(params.emax_ros), float(params.emax_cyto)),
        emission_gamma_per_gy=float(params.gamma),
        emission_tensor=emission_tensor,
        uptake_tensor=uptake_tensor,
        hazard_weights=(float(params.weight_ros), float(params.weight_cyto)),
    ...
    final_survival = calculate_phase7_survival(
        ...
        weight_immune=float(params.weight_immune),
        verbose=False,
    )
