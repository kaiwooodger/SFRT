#!/usr/bin/env python3
"""Phase 36B: uncertainty overlay for true-vs-surrogate sink comparisons on the repeated subset."""
...
from phase36_sink_falsification_utils import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--phase36a-root",
        type=Path,
        default=root / "runs" / "phase36a_vessel_falsification_cohort",
    )
    parser.add_argument(
        "--phase25-run-root",
        type=Path,
    ...
    parser.add_argument(
        "--out-root",
        type=Path,
        default=root / "runs" / "phase36b_vessel_falsification_uncertainty",
    )
...
def build_baseline_subset_model_rows(phase36a_root: Path, *, case_ids: Sequence[str]) -> List[Dict[str, object]]:
    rows = [
        row
        for row in load_csv_rows(phase36a_root / "phase36a_endpoint_table.csv")
        if str(row["plan_id"]) in set(case_ids) and str(row["model_id"]) in set(PHASE36B_MODEL_IDS)
    ]
    ...
    *,
    ...
    for repeat in repeat_manifest:
        ...
        for row in phase33_rows:
            ...
            for model_id in ("mirror_lr_sink", "uniform_body_sink_mass_matched"):
...
def build_true_vs_surrogate_noise_rows(
    ...
    *,
    ...
    comparator_ids = ("no_sink", "mirror_lr_sink", "uniform_body_sink_mass_matched")
...
def simulate_rank_noise(
    ...
    *,
    ...
    for plan_id, model_rows in sorted(by_plan.items()):
        if {str(row["model_id"]) for row in model_rows} != set(PHASE36B_MODEL_IDS):
            continue
        nominal_rank_by_model = {str(row["model_id"]): int(row["model_rank_within_case"]) for row in model_rows}
        shift_samples: Dict[str, List[float]] = {cid: [] for cid in ("no_sink", "mirror_lr_sink", "uniform_body_sink_mass_matched")}
...
def build_baseline_rank_pair_rows(model_rows: Sequence[Mapping[str, object]]) -> List[Dict[str, object]]:
    ...
    for plan_id, models in sorted(by_plan.items()):
        ...
        for comparator_id in ("no_sink", "mirror_lr_sink", "uniform_body_sink_mass_matched"):
...
def build_quick_assessment(
    ...
    lines = [
        ...
        "- Interpretation: this package tests whether the anatomical sink remains distinguishable from simpler sink surrogates after repeated transport and uptake-sensitivity uncertainty are both considered.",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    baseline_model_rows = build_baseline_subset_model_rows(args.phase36a_root.resolve(), case_ids=case_ids)
    ...
    write_csv(out_root / "phase36b_endpoint_table.csv", endpoint_rows)
    write_csv(out_root / "phase36b_assay_table.csv", assay_rows)
    write_csv(out_root / "phase36b_baseline_subset_model_ranks.csv", baseline_model_rows)
    write_csv(out_root / "phase36b_repeat_model_rank_pairs.csv", repeated_rank_rows)
    write_csv(out_root / "phase36b_true_vs_surrogate_noise_table.csv", noise_rows)
    write_csv(out_root / "phase36b_true_vs_surrogate_empirical_rank_summary.csv", empirical_rank_rows)
    write_csv(out_root / "phase36b_true_vs_surrogate_rank_noise.csv", simulated_rank_rows)
    write_json(
        out_root / "phase36b_reproducibility_manifest.json",
        {
            "phase": "36B",
            "description": "Repeated-subset uncertainty overlay for true-vs-surrogate sink comparisons.",
            "phase35_root": str(args.phase35_root.resolve()),
            "phase36a_root": str(args.phase36a_root.resolve()),
            ...
            "model_ids": list(PHASE36B_MODEL_IDS),
            "repeat_count": int(len(repeat_manifest)),
        },
    )
    (out_root / "phase36b_quick_assessment.md").write_text(
    ...
    print("=== PHASE 36B VESSEL FALSIFICATION UNCERTAINTY COMPLETE ===")
    print(f"Output root: {out_root}")
    print(f"Noise table: {out_root / 'phase36b_true_vs_surrogate_noise_table.csv'}")
    print(f"Rank noise table: {out_root / 'phase36b_true_vs_surrogate_rank_noise.csv'}")
    print(f"Quick assessment: {out_root / 'phase36b_quick_assessment.md'}")
    return 0
