#!/usr/bin/env python
"""Phase 11A: robustness sweep over spot-weight patterns and vessel topologies."""
...
VESSEL_MODES = ["none", "single", "branched", "dense"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run Phase 11A robustness sweeps over complex-plan spot-weight "
            "heterogeneity and vessel topology."
        )
    )
    parser.add_argument(
        "--outdir",
        ...
        / "analysis_phase11_pathA_sweep",
        help="Directory for Phase 11A outputs.",
    )
    parser.add_argument(
        "--csv",
        type=Path,
...
def vessel_specs_for_mode(mode: str) -> list[dict]:
    base = default_complex_vessel_specs_mm()
    if mode == "none":
        return []
    if mode == "single":
        return [{**base[0]}]
    ...
    if mode == "dense":
        extra = [
            {
                "label": "inferior_cross",
                "radius_mm": 1.0,
                "uptake_rates_in_vessel": (0.05, 0.56),
                "nodes_mm": [(-14.0, -28.0, -70.0), (-4.0, -18.0, -5.0), (10.0, -6.0, 60.0)],
            },
            {
                "label": "medial_cross",
                "radius_mm": 1.0,
                "uptake_rates_in_vessel": (0.05, 0.52),
                "nodes_mm": [(-18.0, 10.0, -65.0), (-6.0, 18.0, 0.0), (12.0, 26.0, 60.0)],
            },
            {
                "label": "right_drain",
                "radius_mm": 1.2,
                "uptake_rates_in_vessel": (0.05, 0.58),
                "nodes_mm": [(26.0, -12.0, -60.0), (18.0, -2.0, 5.0), (8.0, 10.0, 72.0)],
            },
        ]
        return [{**spec} for spec in base] + extra
    raise ValueError(f"Unknown vessel mode: {mode}")
...
def solve_case(
    *,
    ...
    vessel_specs = vessel_specs_for_mode(vessel_mode)
    ...
    if vessel_mode == "none":
        ...
        meta["vessel_specs_mm"] = []
        meta["vessel_voxel_count"] = 0
...
def matrix_from_rows(
    ...
    *,
    ...
    matrix = np.full((len(SPOT_MODES), len(VESSEL_MODES)), np.nan, dtype=np.float32)
    for row in rows:
        i = SPOT_MODES.index(str(row["spot_mode"]))
        j = VESSEL_MODES.index(str(row["vessel_mode"]))
...
def plot_metric_matrix(
    ...
    *,
    ...
    ax.set_xticks(np.arange(len(VESSEL_MODES)), labels=VESSEL_MODES)
    ax.set_yticks(np.arange(len(SPOT_MODES)), labels=SPOT_MODES)
    ax.set_xlabel("Vessel topology")
    ax.set_ylabel("Spot-weight mode")
    ax.set_title(title)
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            value = matrix[i, j]
            ax.text(j, i, format(float(value), text_fmt), ha="center", va="center", color="#ffffff")
...
def plot_risk_scatter(
    ...
    *,
    ...
    marker_map = {"uniform": "o", "nominal": "s", "core_hot": "^"}
    ...
    for row in rows:
        ax.scatter(
            ...
            marker=marker_map[str(row["spot_mode"])],
            edgecolor="#1a1a1a",
            linewidth=0.7,
        )
        ax.text(
            float(row["center_survival_total"]) + 0.004,
            float(row["valley_eud_shift_gy"]) + 0.015,
            f"{row['spot_mode']}/{row['vessel_mode']}",
            fontsize=7,
        )
    ...
    ax.set_title("Figure 3: Phase 11A safe vs unsafe geometry map")
    ax.grid(alpha=0.25)
    fig.savefig(out_file, dpi=dpi)
    plt.close(fig)
...
def main() -> int:
    ...
    for spot_mode in SPOT_MODES:
        for vessel_mode in VESSEL_MODES:
            print(f"\n=== Phase 11A case: spot={spot_mode}, vessel={vessel_mode} ===", flush=True)
            row = solve_case(spot_mode=spot_mode, vessel_mode=vessel_mode, args=args)
            print(
    ...
    metrics_csv = args.outdir / "phase11_pathA_metrics.csv"
    summary_json = args.outdir / "phase11_pathA_summary.json"
    fig1 = args.outdir / "figure1_phase11_center_survival_matrix.png"
    fig2 = args.outdir / "figure2_phase11_valley_eud_shift_matrix.png"
    fig3 = args.outdir / "figure3_phase11_safe_unsafe_map.png"

    write_csv(rows, metrics_csv)
    plot_metric_matrix(
        matrix_from_rows(rows, value_key="center_survival_total"),
        title="Figure 1: Phase 11A center survival matrix",
    ...
    plot_metric_matrix(
        ...
        title="Figure 2: Phase 11A valley EUD shift matrix",
    ...
    summary = {
        "phase": "Phase 11A",
        "description": "Synthetic robustness sweep over irregular spot-weight patterns and vessel topologies.",
        "sweep_axes": {
    ...
    print(f"\nPhase 11A metrics: {metrics_csv}")
    print(f"Phase 11A summary: {summary_json}")
    return 0
